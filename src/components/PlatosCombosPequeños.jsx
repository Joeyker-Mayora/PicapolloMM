...
const PlatosCombosPequeños = ({ onSeleccion }) => {
  ...
  const {platosSeleccionados, setPlatosSeleccionados, aumentarCantidad, disminuirCantidad, eliminarItem } = useItems("platosPequeños");
  ...
  useEffect(() => {
  try {
    sessionStorage.setItem("platosPequeños", JSON.stringify(platosSeleccionados));
    onSeleccion(platosSeleccionados)
  } catch (e) {
    console.error("Error guardando platosPequeños en localStorage", e);
  }
...
useEffect(() => {
 ...
 const cerrarModal = () => {
  ...
  const permitirChecks = combosPequeños[nombre]?.permitirBebida;
  ...
  {platosSeleccionados.map((plato) => {
    const mostrarChecks = combosPequeños[plato.nombre]?.permitirBebida;
  ...
  {modalDescripcionInfo && (() => {
    const nombre = modalDescripcionInfo.nombre;
    const data = combosPequeños[nombre] || {};
      ...
      <div className="relative p-4 sm:p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="absolute right-1 font-extrabold -top-1 sm:-top-4 text-gray-400 hover:text-orange-600 text-2xl leading-none"
            aria-label="Cerrar"
          >
            ✕
          </button>
        ...
        {/* Info general */}
        {data.piezas && (
          <p className="text-sm font-semibold text-gray-900">{data.piezas}</p>
        )}
        {data.tipo && (
          <p className="text-sm text-gray-700 mt-0.5">{data.tipo}</p>
        )}
        {data.descripcion && (
        ...
        {data.permitirBebida && (
          <div className="mt-3">
            ...
            <CheckBebida platoId={modalDescripcionInfo.id} />
          </div>
        )}
        ...
        <div className="flex justify-between items-center">
          {/* Precio siempre en esquina inferior izquierda */}
          <div className="text-lg font-semibold text-orange-600">
            {data.precio ? formatUSD(data.precio) : null}
          </div>
...
export default PlatosCombosPequeños;