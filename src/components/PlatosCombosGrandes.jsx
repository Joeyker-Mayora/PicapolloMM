...
const opcionesPlatos = Object.keys(combosGrandes).map((item) => ({
  label: item,
  value: item,
}));
...
const PlatosCombosGrandes= ({ onSeleccion }) => {
  ...
  useEffect(() => {
  try {
    sessionStorage.setItem("PlatosGrandes", JSON.stringify(platosSeleccionados));
    onSeleccion(platosSeleccionados)
  } catch (e) {
    console.error("Error guardando PlatosGrandes en localStorage", e);
  }
...
useEffect(() => {
  ...
  const precio = combosGrandes[platoNombre]?.precio || "0";
 ...
 const cerrarModal = () => {
  ...
  const permitirChecks = combosGrandes[nombre]?.permitirBebida;
...
const platoConBebida = { 
  ...
  {platosSeleccionados.map((plato) => {
    const mostrarChecks = combosGrandes[plato.nombre]?.permitirBebida;
   ...
   <ModalAnimado isOpen={!!modalDescripcionInfo} onClose={cerrarModal}>
     {modalDescripcionInfo && (() => {
       const nombre = modalDescripcionInfo.nombre;
       const data = combosGrandes[nombre] || {};
...
export default PlatosCombosGrandes;